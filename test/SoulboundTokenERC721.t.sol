...
import {SoulboundTokenERC721} from "../src/token/SoulboundTokenERC721.sol"; // adjust if your path differs
import {ITokenERC721} from "@thirdweb-dev/contracts/prebuilts/interface/token/ITokenERC721.sol";

contract SoulboundTokenERC721Test is Test {
    ...
    function setUp() public {
        ...
        proxyAddress = address(new SoulboundTokenERC721());
        ...
        // deploy the proxy
        bytes32 contractType = SoulboundTokenERC721(proxyAddress)
            .contractType();
        ...
        SoulboundTokenERC721(deployedAddr).grantRole(MINTER_ROLE, minter);
        vm.stopPrank();
    }
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    keccak256(bytes("SoulboundTokenERC721")),
                    keccak256(bytes("1")),
                    block.chainid,
    ...
    function test_mintWithSignature_respectsPerWalletLimit() public {
        ...
        SoulboundTokenERC721 token = SoulboundTokenERC721(deployedAddr);
        ...
        vm.prank(user);
        vm.expectRevert(SoulboundTokenERC721.AlreadyMinted.selector);
    ...
    function test_untransferable() public {
        SoulboundTokenERC721 token = SoulboundTokenERC721(deployedAddr);
        vm.prank(user);
        vm.expectRevert();
        token.transferFrom(user, address(0x123), 1);
    }
}
